...
export class SkyscrapersBackground implements IBackground {
	constructor(
	...
	private operatePoint(x: number, y: number) {
		...
		const selectedPoints: number[] = nearbyPointsValue
			...
			.filter((value) => isInRange(Math.abs(pointValue - value), 1, 3));

		if (selectedPoints.length >= 4) this.screen.setValue(x, y, selectedPoints[0]);
	}
